...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.50]{diagrams/analysis/death_icu.png}
    \caption{The rate of death per capita vs the ICU occupancy rate with a line fit through the data using least squares}
...
\section{Conclusions}
In this work we designed a database for keeping a history of COVID-19 data (revise). We demonstrated that in order to build the history, our design is able to incorporate data from multiple sources. We also showed that the data we store allows for statistical studies to unveil COVID-19 related trends and correlations. Adding the timestamps and using natural identifiers for the records allowed us to easily combine data gathered from different sources in our sample analyses. If in order to perform an analysis, it becomes necessary to incorporate other sources into the database, the new data can be easily integrated into our database to be used in conjunction with the existing data.\\

\noindent
As we planned and executed different phases of this project, we realized that the most involved and sensitive part of the project was designing the database. In other words, once a good database design was achieved, retrieving data and performing analyses on the data was fairly straightforward. 
