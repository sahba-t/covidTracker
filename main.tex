...
\noindent
The data was collected between 5 MST and 7MST everyday from May 6, 2020 until May 13, 2020 using the REST API and a python script (GatherData.py). This process involved posting a GET request to the 5 endpoints described in Section \ref{subsec:sourcedata} and obtaining the data in JSON format. 
...
\noindent
This allowed the users to run the python script once every evening to collect and organize data, and subsequently upload the data into the Oracle Cloud Database instance. Backups of the raw data obtained from the COVID-19 API endpoints, in addition to backups of the processed data that was prepared for database insertion were kept everyday to ensure that no data would be lost in the unlikely case of an accident or data loss. Additionally, the data, python scripts, and SQL scripts were all kept in sync and backed up on GitHub (which also allowed easy collaboration).
...
\noindent
We mainly used SQL queries and Python's data science toolset (Pandas\cite{citepandas}, NumPy\cite{citenumpy}, ScikitLearn\cite{cite_scikit-learn}) to retrieve, prepare,  and analyze the our data. The following sections provides details on our approach in preparing the data and performing the analysis.
